...
class StaticVisuals extends React.Component {
    ...
    alertMessage = () => {
        const key = this.props.enqueueSnackbar("Please select a maximum of up to 15 countries", {
            preventDuplicate: true,
            variant: 'error',
        })
        return key;
    ...
    renderSeverityRadios() {
        var radios = [];
        for (let i = 0; i < this.state.lists.severityList.length; i++) {
            radios.push(
                <FormControlLabel value={this.state.lists.severityList[i]} control={<Radio />} label={this.state.lists.severityList[i]} />
            )
        }
        return radios;
    }
